...
**Teste rápido**: existe uma variante chamada _contextualized chunk embeddings_ — em vez de escanear cada pedacinho do vault isoladamente, o scanner olha o **documento inteiro** enquanto gera a etiqueta de cada pedaço, pra etiqueta ficar mais "ciente do contexto ao redor". Pensando no que você aprendeu na seção 1 (scanner = passada única, sem eixo de output): você acha que isso mudaria o **preço** do escaneamento, ou só a **qualidade** da etiqueta final? Por quê?
 ...
 - Boa intuição, mas precisa de um ajuste fino — parecido com aquela pegadinha da Aula 1 (Big-O vs. custo real). 
   **Sim, tecnicamente rola mais trabalho computacional por trás**, porque o scanner agora olha o documento inteiro enquanto gera a etiqueta de cada pedacinho, não só o pedacinho isolado. Isso é mais cálculo _internamente_.
	- **Mas isso não vira preço mais alto pra você.** Continua sendo **uma única passada** (o scanner não "escreve" nada, só mede) e continua sendo cobrado pela mesma régua: tokens processados, mesma tarifa por milhão. A própria Voyage é explícita sobre isso — o `voyage-context-4` é descrito como "mais simples, mais rápido e **mais barato**" que a alternativa antiga (que era usar uma LLM pra escrever um resumo manual em cima de cada chunk antes de escanear — isso sim gerava um custo extra real, de tokens de _geração de texto_, que é caro). Então a distinção fica assim:

|                             | O que muda                                                                                                                                           |
| --------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| Preço pago por você         | Praticamente nada — mesma tarifa por token, mesma lógica de passada única                                                                            |
| Qualidade da etiqueta       | Melhora bastante — o vetor de cada pedaço já "sabe" de onde veio, sem você precisar inserir manualmente aqueles separadores `[Fonte: ...]` da Aula 2 |
| Trabalho interno do scanner | Aumenta (ele processa mais contexto por trás), mas isso é problema da Voyage otimizar, não seu bolso                                                 |

- Ou seja: seu instinto de "mais contexto = mais trabalho" está certo — só que aqui esse trabalho extra fica **absorvido dentro da mesma tarifa**, porque o "eixo caro" que você conhece (output sequencial) simplesmente não existe em embedding. É como se o scanner fizesse uma análise mais cuidadosa da foto, mas ainda tirasse **uma foto só** — você paga pelo clique, não pela cautela do scanner.
- **Pergunta pra fechar**: dado isso, faz sentido usar `voyage-context-4` sempre, por padrão, no lugar do modelo padrão — já que o preço não muda mas a qualidade melhora? Tem algum cenário do seu vault onde isso _não_ valeria a pena?